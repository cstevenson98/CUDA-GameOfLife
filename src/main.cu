...
void rle2state(std::string &rle, std::vector<unsigned int> &in, int x, int y) {
  ...
  for (auto i = rle.begin(); i != rle.end(); ++i) {
    ...
    if (*i == 'b' || *i == 'o') {
      for (int j = 0; j < num_to_write; j++) in.push_back((*i == 'o'));
    }
    // If an endl,
    else if (*i == '$') {
      for (int j = 0; j < x * (num_to_write - 1); j++) in.push_back(0);
    }
    currentNums = "";
  }
...
int main(void) {
  ...
  window = glfwCreateWindow(widthX * pointSize, widthY * pointSize, WindowTitle,
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      ImGui::NewFrame();
      {
        ...
        ImGui::Begin("Settings");  // Create a window called "Hello, world!" and
                                   // append into it.
        ImGui::Text("Choose simulation:");  // Display some text (you can use a
                                            // format strings too)
        ImGui::RadioButton("Game of Life", (int *)&currentSim, GOL);
        ImGui::RadioButton("Maxwell", (int *)&currentSim, MAXWELL);

        if (ImGui::Button(RUN_SIMULATION ? "Pause" : "Run") ||
