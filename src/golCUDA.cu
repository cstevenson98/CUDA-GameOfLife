...
#include "gol/golCUDA.h"

__global__ void GolKernel_random(unsigned int *cellData, float density,
                                 int seed) {
...
__global__ void GolKernel_next(unsigned int *cellData, unsigned int *cellNext) {
  ...
  if ((xId < (gridDim.x * blockDim.x - 1)) && (xId > 0) && (yId > 0) &&
      (yId < (gridDim.y * blockDim.y - 1))) {
    unsigned int sum = 0;
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
